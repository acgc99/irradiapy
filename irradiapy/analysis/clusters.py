...
def get_clusters_0d(
    ...
    scale : float, optional
        All bin counts are multiplied by this factor. Useful for normalization. Default is `1.0`.
    """
...
def plot_clustering_fraction(
    ...
    db_path : Path
        ...
        `irradiapy.analysis.clusters.get_clusters_1d`.
    path_sias : Path, optional
        Path to save the interstitials clustering fraction plot. If `None`, the plot is shown
    ...
    def plot(
        ...
        histogram *= size_centers
        clustering_fraction = np.sum(histogram[:, 1:], axis=1) / histogram[:, 0]
        ...
        ax.plot(depth_centers, clustering_fraction, marker="o", linestyle="none")
        ax.set_ylim(0, 1)
        ax.set_xlim(min_depth, max_depth)
    ...
    interstitials = data["interstitials"]
    vacancies = data["vacancies"]
    plot(depth_edges, interstitials, "Interstitials", path_sias)
    plot(depth_edges, vacancies, "Vacancies", path_vacs)
...
def plot_mddb_cluster_size_distribution(
    ...
    sia_bin_width : int, optional
        Bin size for interstitial histogram. Defaults to 10.
    vac_bin_width : int, optional
        Bin size for vacancy histogram. Defaults to 10.
    path_sias : Path, optional
        Path to save the interstitials size distribution plot. If `None`, the plot is shown instead
        of saved. Default is `None`.
    path_vacs : Path, optional
        Path to save the vacancies size distribution plot. If `None`, the plot is shown instead
        of saved. Default is `None`.
    max_sia : int, optional
        ...
        compare multiple databases with different maximum sizes. Default: `None`.
    max_vac : int, optional
        ...
        compare multiple databases with different maximum sizes. Default: `None`.
    vmin : float, optional
        Minimum value for the color scale if `rel` is `True`. Default is 1e-5.
    """
